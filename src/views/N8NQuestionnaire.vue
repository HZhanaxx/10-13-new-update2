<template>
  <div class="n8n-questionnaire-page">
...
// Computed
const canGoBack = computed(() => questionHistory.value.length > 0 && !isSubmitting.value)
...
const getSessionFromStorage = () => {
  const stored = sessionStorage.getItem('n8n_questionnaire_session')
  if (stored) {
...
const initQuestionnaire = async () => {
  ...
  try {
    sessionInfo.value = getSessionFromStorage()
    ...
    const response = await api.post('/n8n/questionnaire/webhook/start', 
      {
        action: 'start',
        templateType: 1,
      ...
      {
        headers: { Authorization: `Bearer ${sessionInfo.value.jwtToken}` }
      }
    )
    ...
    sessionInfo.value.sessionId = data.sessionId
    currentQuestion.value = data.question
    progress.value = data.progress
    partInfo.value = data.partInfo
...
const submitAnswer = async () => {
  ...
  try {
    ...
    // ✅ Send JWT in Authorization header
    const response = await api.post('/n8n/questionnaire/webhook/answer', 
      {
        action: 'answer',
        ...
        answer: answerValue,
        sessionData: {
          answers: questionHistory.value.reduce((acc, h) => {
            acc[h.question.id] = { value: h.answer }
            return acc
          }, {})
        },
        timestamp: new Date().toISOString()
    ...
    if (data.completed) {
      ...
      sessionStorage.removeItem('n8n_questionnaire_session')
    } else if (data.showSummaryReview) {
      showSummaryReview.value = true
      currentSummary.value = data.summary
      currentPart.value = data.currentPart
    } else if (data.transitionMessage) {
      transitionMessage.value = data.transitionMessage
...
const requestRegenerate = async () => {
  ...
  try {
    // ✅ Send JWT in Authorization header
    const response = await api.post('/n8n/questionnaire/webhook/summary-regenerate', 
      {
        sessionId: sessionInfo.value.sessionId,
...
const processFile = async (file) => {
  ...
  reader.onload = async () => {
    ...
    // ✅ Send JWT in Authorization header
    try {
      const response = await api.post('/n8n/questionnaire/webhook/file-upload', 
        {
          sessionId: sessionInfo.value.sessionId,
          questionId: currentQuestion.value.id,
          file: {
...
const goBack = () => {
  if (confirm('确定要退出问卷吗？已填写的内容将不会保存。')) {
    sessionStorage.removeItem('n8n_questionnaire_session')
...
@media (max-width: 768px) {
  .n8n-questionnaire-page { padding: 16px; }
